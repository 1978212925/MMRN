...
def train_cla_OOD_epoch(model, loss_fn, optimizer, data_loader, device):
    ...
    for data in tqdm(data_loader):
        ...
        optimizer.zero_grad()
        y_pred, OOD_stages = model(imgs)
        loss = loss_fn(y_pred, labels, OOD_stages)
...
def val_cla_OOD_epoch(model, loss_fn, optimizer, data_loader, device):
    metric = ClassificationMetric(2)
    model.eval()
    running_loss = 0
    y_true = []
    ...
    for data in tqdm(data_loader):
        ...
        labels = labels.to(device)
        optimizer.zero_grad()
        y_pred, gram = model(imgs)
        loss = loss_fn(y_pred, labels, gram)
        y_true.append(labels.cpu().numpy())
        y_score.append(y_pred[:, 1].cpu().detach().numpy())
        with torch.no_grad():
            running_loss += loss.item()
